...
contract DSCEngineTest is StdCheats, Test {
    ...
    function test_GetAccountInformationAfterDeposit() public depositedCollateral {
    ...
    function test_RevertsIfMintedDscBreaksHealthFactor() public {
        ...
        amountToMint = (collateralValueInUsd * 60e18) / dscEngine.getPrecision();
    ...
    function test_RevertIfBurnAmountIsZero() public depositedCollateralAndMintedDsc {
        vm.startPrank(user);
        vm.expectRevert(abi.encodeWithSelector(DSCEngine.DSCEngine__AmountMustBeGreaterThanZero.selector));
        dscEngine.burnDsc(0);
        vm.stopPrank();
    ...
    function test_CannotBurnMoreThanUserBalance() public depositedCollateralAndMintedDsc {
        vm.startPrank(user);
        uint256 userBalance = deStablecoin.balanceOf(user);
    ...
    function test_RevertsIfTransferFails() public {
        ...
        vm.expectRevert(abi.encodeWithSelector(DSCEngine.DSCEngine__TransferFailed.selector));
        mockDscEngine.redeemCollateral(address(mockCollateral), amountCollateral);
        vm.stopPrank();
    }
    ...
    function test_CanRedeemDepositedCollateral() public depositedCollateralAndMintedDsc {
        vm.startPrank(user);
    ...
    function test_ProperlyReportsHealthyAccount() public depositedCollateralAndMintedDsc {
        uint256 expoectedHealthFactor = dscEngine.getHealthFactor(user);
        uint256 healthFactor = dscEngine.getHealthFactor(user);
        assertEq(healthFactor, expoectedHealthFactor);
    }

    function test_HealthFactorCanGoBelowMinimum() public depositedCollateralAndMintedDsc {
    ...
    function test_MustImproveHealthFactorOnLiquidation() public depositedCollateralAndMintedDsc {
    ...
    function test_CantLiquidateGoodHealthFactor() public depositedCollateralAndMintedDsc {
        ...
        vm.expectRevert(abi.encodeWithSelector(DSCEngine.DSCEngine__HealthFactorIsOk.selector));
    ...
    function test_UserStillHasSomeEthAfterLiquidation() public liquidated {
        uint256 userWethBalance = dscEngine.getCollateralBalanceOfUser(user, weth);
